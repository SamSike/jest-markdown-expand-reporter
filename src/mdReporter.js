"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const os_1 = __importDefault(require("os"));
const mdGenerator_1 = __importDefault(require("../dist/mdGenerator"));
var LogOptions;
(function (LogOptions) {
    // Display console logs in the report
    LogOptions["ALL_LOGS"] = "all";
    LogOptions["DEBUG"] = "debug";
    LogOptions["ERROR"] = "error";
    LogOptions["INFO"] = "info";
    LogOptions["LOG"] = "log";
    LogOptions["WARN"] = "warn";
})(LogOptions || (LogOptions = {}));
class MDReporter {
    constructor(globalConfig, reporterOptions) {
        this.displayAllTests = false;
        this.failureMessages = true;
        this.logs = {};
        this.globalConfig = globalConfig;
        this.options = reporterOptions || {};
        this.filename = this.options.filename || 'test-results.md';
        this.publicPath = this.options.publicPath || './';
        this.consoleLogs = this.options.consoleLogs || [];
        this.displayAllTests = this.options.displayAllTests || false;
        this.failureMessages = this.options.failureMessages || true;
        this.startTime = new Date();
        this.logs = {};
    }
    getOptions() {
        return {
            consoleLogs: this.consoleLogs,
            displayAllTests: this.displayAllTests,
            failureMessages: this.failureMessages,
        };
    }
    async onTestResult(test, testResult) {
        this.logs[test.fullName || test.title] = testResult.console || [];
    }
    async onRunComplete(contexts, runResults) {
        const tmpDir = os_1.default.tmpdir();
        const enabledLogTypes = (this.consoleLogs || []).map((type) => type.toLowerCase());
        // For each test suite, read its corresponding log file
        for (const suite of runResults.testResults) {
            const suiteFile = suite.testFilePath
                ? path_1.default.basename(suite.testFilePath, path_1.default.extname(suite.testFilePath))
                : 'unknown-suite';
            const logFile = path_1.default.join(tmpDir, `jest-md-logs-${suiteFile}.json`);
            let suiteLogs = {};
            if (fs_1.default.existsSync(logFile)) {
                try {
                    suiteLogs = JSON.parse(fs_1.default.readFileSync(logFile, 'utf-8'));
                }
                catch (e) {
                    process.stderr.write(`Could not parse log file ${logFile}: ${e}\n`);
                }
            }
            // Attach logs to each test in the suite, filtering by enabled log types
            for (const test of suite.testResults) {
                const testName = test.fullName || test.title;
                let logs = suiteLogs[testName] || [];
                // Only include logs if enabled in options
                if (enabledLogTypes.includes('all') ||
                    enabledLogTypes.some((type) => logs.some((log) => (log.type || '').toLowerCase() === type))) {
                    // If not 'all', filter logs by enabled types
                    if (!enabledLogTypes.includes('all')) {
                        logs = logs.filter((log) => enabledLogTypes.some((type) => (log.type || '').toLowerCase() === type));
                    }
                    test.consoleLogs = logs;
                }
                else {
                    test.consoleLogs = [];
                }
            }
        }
        // Prepare data for the EJS template
        const data = {
            packageName: this.globalConfig.rootDir ? path_1.default.basename(this.globalConfig.rootDir) : 'Project',
            date: this.startTime,
            testResults: runResults.testResults,
            ...this.getOptions(),
        };
        // Generate the markdown report using gen.ejs
        try {
            const report = await mdGenerator_1.default.generate(data, data.date);
            if (!fs_1.default.existsSync(this.publicPath))
                fs_1.default.mkdirSync(this.publicPath, { recursive: true });
            const filename = path_1.default.join(this.publicPath, this.filename);
            fs_1.default.writeFileSync(filename, report);
            process.stdout.write(`Markdown report generated at: ${filename}\n`);
        }
        catch (err) {
            process.stderr.write(`Markdown report generation failed: ${JSON.stringify(err, null, 2)}\n`);
            process.exitCode = 1;
        }
    }
}
exports.default = MDReporter;
